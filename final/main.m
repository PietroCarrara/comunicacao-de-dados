...
NUM_BITS = 10008;                                                        % número de bits de dados a serem simulados
...
QPSK_DATA = encode_qpsk(RAW_DATA); % bits aleatórios modulados em QPSK (partes em 1 e -1)
...
for i = 1:length(Eb_N0_lin)
    % QPSK
    n = NA_QPSK(i)*complex(randn(1, length(QPSK_DATA)), randn(1, length(QPSK_DATA)))*sqrt(0.5); % vetor de ruído complexo com desvio padrão igual a uma posição do vetor NA
    r = QPSK_DATA + n;                                                        % vetor recebido
    QPSK_DEMOD = decode_qpsk(r);                       % QPSK: corrige a informação pra compensar pelo ruido (pode ser que corrija errado)
    QPSK_BER(i) = sum(RAW_DATA ~= QPSK_DEMOD(1:length(RAW_DATA))) / length(RAW_DATA);                    % contagem de erros e cálculo do BER

    % 64-QAM
    n = NA_QAM(i)*complex(randn(1, length(QAM_DATA)), randn(1, length(QAM_DATA)))*sqrt(0.5);
    r = QAM_DATA + n;
    QAM_DEMOD = decode_qam(r);                                                % 64-QAM: corrige a informação pra compensar pelo ruido (pode ser que corrija errado)
    QAM_BER(i) = sum(RAW_DATA ~= QAM_DEMOD(1:length(RAW_DATA))) / length(RAW_DATA);                % contagem de erros e cálculo do BER
    
    % QPSK + convolution coding 2/3
    n = NA_QPSK(i)*complex(randn(1, length(CONV_QPSK_DATA_2_3)), randn(1, length(CONV_QPSK_DATA_2_3)))*sqrt(0.5); % vetor de ruído complexo com desvio padrão igual a uma posição do vetor NA
    r = CONV_QPSK_DATA_2_3 + n;                                                        % vetor recebido
    QPSK_DEMOD = vitdec(decode_qpsk(r), TRELLIS, 32, 'trunc', 'hard', TWO_THIRDS_PUNCTURE);
    CONV_2_3_QPSK_BER(i) = sum(RAW_DATA ~= QPSK_DEMOD(1:length(RAW_DATA))) / length(RAW_DATA);                    % contagem de erros e cálculo do BER
    ...
    n = NA_QPSK(i)*complex(randn(1, length(CONV_QPSK_DATA_3_4)), randn(1, length(CONV_QPSK_DATA_3_4)))*sqrt(0.5); % vetor de ruído complexo com desvio padrão igual a uma posição do vetor NA
    r = CONV_QPSK_DATA_3_4 + n;                                                        % vetor recebido
    QPSK_DEMOD = vitdec(decode_qpsk(r), TRELLIS, 32, 'trunc', 'hard', THREE_FOURTHS_PUNCTURE);
    CONV_3_4_QPSK_BER(i) = sum(RAW_DATA ~= QPSK_DEMOD(1:length(RAW_DATA))) / length(RAW_DATA);                    % contagem de erros e cálculo do BER
