...
QAM_DATA = complex(2*randi(8, 1, NUM_BITS)-9, 2*randi(8, 1, NUM_BITS)-9);  % bits aleatórios modulados em 64QAM (valores ímpares entre 7 e -7)
...
ENERGY_PER_BIT_QAM = mean(sqrt(real(QAM_DATA).^2 + imag(QAM_DATA).^2)); % energia média por bit do QAM (x^2 + y^2 = tamanho_do_vetor^2)
...
for i = 1:length(Eb_N0_lin)
    ...
    % 64-QAM
    n = NA_QAM(i)*complex(randn(1, NUM_BITS), randn(1, NUM_BITS))*sqrt(0.5); % vetor de ruído complexo com desvio padrão igual a uma posição do vetor NA
...
% Ainda tendando ver se o errado pro QAM é a fórmula ou a implementação
k=6;
M=2^k;
x=sqrt(3*k*Eb_N0_lin/(M-1));
...
semilogy(Eb_N0_dB, QAM_BER, 'x', Eb_N0_dB, Pb, 'r', 'LineWidth', 2, 'MarkerSize', 10);
